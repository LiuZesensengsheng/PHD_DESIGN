...
# 游戏设计文档：甘特方块模型 (Gantt-tris Model)权威
...
## 2. 核心概念：任务线 (Task Lines) 权威一下

游戏由0-n条多条任务线组成，每个任务线代表这一个长期的任务，该任务具有 以下属性 1 完成度 2 总量 3 腐化度。
完成度代表着 你对项目的进展，总量则是项目的最多需要做的内容。其中，整体满足以下关系约束：
 完成度<总量。
每个任务导向一次boss战斗，最终的boss强度会受到 完成度，和总量的影响。
...
目的：在不改变核心循环的前提下，将“必须处理的进度块（任务）”与“叠加的黑色幽默段子/微互动（事件）”解耦。

- 任务（Task Block）
  - 消耗一回合（进入战斗/遭遇/交互）；参与融合与奖励结算。
  - 字段：`type=TASK`, `theme`, `enemy_count`, `has_boss`, `duration_turns=1`。
  - 行为：点击进入并处理；处理结果影响轨道进度与后续生成。

- 事件（Event Overlay）
  - 叠加在甘特块上方的小图标，点击弹出文本/二选一；不进入战斗，不消耗回合。
  - 不参与融合/波次计算，不改变主题；仅做轻量叙事和临时效果（如 +自信、+洞见）。
  - 字段：`type=EVENT`, `icon`, `title`, `body`, `choices[]`, `ttl_turns`。

参考规范：`docs/design/campaign/GANTT_TASKS_EVENTS.md`


## 13. 融合规则（MVP 简化：数量融合 + 主题继承）

为降低实现复杂度，当前版本采用简化融合：

- 主题继承：融合战的 `theme` 取“前一个节点”的主题。
- 数量融合：`total_enemies = Σ enemy_count`；同屏上限 5，其余进入第二波（第一波 ≤2 时入场）。
- Boss优先：若任一节点含 Boss → 以该 Boss 为主，余量用小怪填充。
- 奖励倍率：`multiplier = 1 + 0.2 × (参与节点数 - 1)`。
- 事件：事件不参与融合、波次与主题判定。

说明：原“基于标签的涌现式融合”作为长期目标保留；MVP 先以数量融合稳定体验。详见：`docs/design/campaign/GANTT_FUSION_RULES.md`。
