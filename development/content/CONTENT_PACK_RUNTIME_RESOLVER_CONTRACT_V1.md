...
- `contexts/shared/infrastructure/content_pack_runtime_context_guard.py`
  ...
  now five entries after `NarrativeApplicationService`, `RewardService`,
  `PostCombatRewardFlowService`, `build_campaign_state_service_bundle`, and
  `CampaignState` moved to explicit required `ContentPackRuntimeContext`
  inputs. This is report-only visibility; it does not change runtime loading,
  resolver activation, save pinning, hot reload, or UI DLC selection.
