...
Axis-first autonomous design run records are retained as a report-only
library/fixture surface. Their former standalone CLI wrapper is retired from
default routes. Current user-facing runs should enter through programmatic
complete-card draft generation, which writes nested autonomous-run artifacts.

```powershell
python scripts/run_programmatic_complete_card_draft_generation.py --axis-search tests/fixtures/combat_analysis/mechanism_axis_design_brief_v1/silent_axis_search_bundle_snapshot_v1.json --design-brief tests/fixtures/combat_analysis/mechanism_axis_package_seed_v1/silent_axis_design_brief_snapshot_v1.json --package-seed <card_package_proposal_v1.json> --target tests/fixtures/combat_analysis/sts1_exam_target_v1/silent_poison_retain_shiv_exam_target_v1.json --variant-set tests/fixtures/combat_analysis/card_package_variant_set_v1/silent_poison_retain_shiv_variant_set_v1.json --generator-provider deterministic_slot_template_provider_v1 --output-dir tmp/combat_analysis/programmatic_complete_card_draft_generation_current
```
...
The historical mechanism-axis evaluation handoff, owner-report/evidence, and
card-package-health preparation stages are now retained as report-only
library/fixture surfaces only. Their former standalone wrappers are retired
from default routes, because current Codex-facing card package work should
enter through `cardanalysis_tool_facade_v1` actions: generate/ingest, exam,
Chinese review-pack, feedback, and iterate.

Do not use the retired handoff/owner/health wrappers as active command surfaces
when building new card packages. If old fixture evidence is needed for archival
analysis, use the retained toolkit contract tests for the specific library
module instead of reviving a script wrapper.
...
## Retired Owner/Evidence Preparation Chain

The former `mechanism_axis_evaluation_handoff_v1`,
`mechanism_axis_owner_report_*`, `mechanism_axis_generation_exam_readiness_v1`,
`mechanism_axis_owner_evidence_*`,
`mechanism_axis_card_package_health_*`, and
`mechanism_axis_evaluation_evidence_merge_v1` chain is retired from active code.

Current mechanism-axis preparation is intentionally smaller:

1. Build `mechanism_axis_search_bundle_v1`.
2. Build `mechanism_axis_design_brief_v1`.
3. Build `mechanism_axis_package_seed_v1`.
4. Continue through the facade-backed generate/ingest, exam, review-pack,
   feedback, and iterate actions.

The canonical `card_package_health_v1` evaluator remains active, but the old
mechanism-axis owner-filled scaffold/readiness/input-writer/owner-run/evidence
adapter path is no longer a supported route. New package work should not rebuild
that side chain unless a future accepted decision adds it back through the
facade route map.
