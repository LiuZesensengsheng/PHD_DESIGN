...
card_package_draft_handoff_v1
  ...
  + optional llm_draft_prompt_application_v1 intake snapshots
...
When `--prompt-application` snapshots are supplied, the batch derives ordered
draft paths from those intake records and records their boundary-preservation
summary in the top-level snapshot and manifest.
...
Prompt-application inputs are also ordered. Each intake snapshot must point to
the same handoff and its `draft_input_path` must match the corresponding draft
file. Any intake snapshot that claims LLM runtime calls, generated runtime card
data, reviewed evidence, hard gates, default synthesis changes, or learned/reranker
